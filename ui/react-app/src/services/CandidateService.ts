...
class CandidateService {
  ...
  public static async editCandidate(
    ...
    const res = await apiClient.put(`${url}/${id}`, candidate, { retry: 1 } as any);
    return res.data;
  }

  public static async deleteCandidate(id: string | undefined) {
    const url = Api.BASE_URL + Api.DELETE_CANDIDATE;
    const res = await apiClient.delete(`${url}/${id}`, { retry: 1 } as any);
    return res.data;
  }
}
